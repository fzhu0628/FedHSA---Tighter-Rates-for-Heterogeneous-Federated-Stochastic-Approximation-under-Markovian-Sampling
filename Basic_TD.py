...
for round in range(T):
    ...
    for i in range(num_agents):
        ...
        for _ in range(H):
            g = phi_all[i].T @ D_all[i] @ (R_all[i] + gamma[i] * P_all[i] @ phi_all[i] @ x_local - phi_all[i] @ x_local)
...
for _ in range(T):
    x_local_list = []    
    g_begin_global = np.zeros((r, 1))
    g_begin_list = []
    for i in range(num_agents):
        g_begin = phi_all[i].T @ D_all[i] @ (R_all[i] + gamma[i] * P_all[i] @ phi_all[i] @ x_global - phi_all[i] @ x_global)
...
plt.plot(norm_differences1,'-^', color='r', linewidth=2,markevery=50, label="FedHSA")
